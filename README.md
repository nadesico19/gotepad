...
- SGF 棋谱读写：支持棋谱信息、预置节点、分支、局面标题、评论、常用棋盘标记和 Gotepad 自定义数据，并在加载异常棋谱时提供防御性校验与提示。
- 棋局笔记：同一局面可保存多层笔记，支持顺序字母、三角、方形、圆形和叉形标记，并可为阅读及出版设置不同的棋子编号方式。
...
- Android 版既可以在程序内通过系统文件选择器加载 SGF，也可以从文件管理器或其他支持系统“打开方式”的应用直接使用 Gotepad 打开 SGF。
