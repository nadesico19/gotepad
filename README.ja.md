...
このソフトは、囲碁のメモ帳という位置づけであるため、Notepad にちなんで Gotepad と名付けました。機能的にはメモ帳に留まらず、KataGo による解析や人間模倣プレイなども実装されています。

人間模倣プレイは専用モデルが必要なので、 Release には比較的に新しいものをアプリ本体と一緒に入れました。特に最新版にこだわらなければ、合わせてダウンロード願います。

初期状態の言語は中国語に固定してありますが、右上の「设置」パネルの「界面语言」をいじれば、「日本語」に切り替えられます。

Android アプリは KataGo エンジンと 10b モデルが内蔵され、すぐに使えるため、特におすすめします。Windows では自前で KataGo 環境を用意する必要があるものの、今や AI エージェントに頼めば苦労せずにできるので、そちらもぜひお試しください。

MacOS 版はビルドする環境が整う次第、リリースする予定です。iOS アプリに関しては、無料に徹するつもりで、出し方にはまだ悩んでいます。

以降の内容及びマニュアル類は、全部 Gemni 先生に頼んで訳したものです。多少の読みづらさを承知の上、アプリをご利用の際に、ご参考いただければ幸いです。
...
- KataGo 解析：単一局面・継続解析、候補手と変化図、盤面上の追加候補数、候補マーカーの不透明度、勝率・目差曲線、棋譜全体の高速解析、探索量の追加、実戦手の劣化警告に対応します。上位 3 候補とその他の候補は、不透明度を個別に設定できます。Windows はユーザーが配置した外部エンジンを使用し、Android は OpenCL/Eigen バックエンドとモデルを内蔵するとともに、外部モデルも読み込めます。
...
- [OpenCV](https://github.com/opencv/opencv)：画像内の碁盤のグリッド検出、透視補正、黒石・白石の認識に使用するコンピュータビジョンアルゴリズム。Apache License 2.0 で提供されます。
